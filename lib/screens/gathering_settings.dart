...
class _GatheringSettingsScreenState extends State<GatheringSettingsScreen> {
              ...
              children: [
                ...
                _card('일정표', [
                  const Text(
                    ...
                    '토글을 끈 날은 일정 없는 날로, 신청자가 부분 참석 날짜로 고를 수 없습니다.',
                    style: TextStyle(fontSize: 12.5, color: AppColors.textMuted),
                  ),
                  const SizedBox(height: 4),
                  for (final d in x.days) _scheduleDay(x, d),
                ]),
                _card('이미지', [
                  Wrap(
                    spacing: 20,
  ...
  Widget _scheduleDay(Gathering x, DateTime d) {
    // 끈 날(일정 없는 날)의 일정 줄은 숨기기만 한다 — 다시 켜면 그대로 돌아온다.
    final off = x.isOff(d);
    final mine = [
      if (!off)
        for (final l in lines) if (dateOnly(l.date) == dateOnly(d)) l,
    ];
    return Padding(
        ...
        children: [
          ...
          Switch(
            ...
            onChanged: (on) => setState(
              () => on
                  ? x.offDays.removeWhere((o) => dateOnly(o) == dateOnly(d))
                  : x.offDays.add(dateOnly(d)),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
