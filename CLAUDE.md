...
공지 링크(`?g=<집회id>&n=<공지id>`)로 들어오면 신청 웹이 그 공지를 맨 위에 펼쳐 준다.
집회 설정의 `kakaoChannelUrl`(카카오톡 채널 = 옛 플러스친구, 선택)을 넣으면 신청 완료 화면·집회 페이지·공지 메시지에 [채널 추가] 안내가 붙는다 — 비우면 아무 데도 안 나온다. 무료로 더 갈 수 있는 길(브라우저 푸시)은 `PLAN_NOTICE.md` §5 에 검토만 해 두었다.
