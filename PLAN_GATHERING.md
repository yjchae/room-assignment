...
- **가정 배정 창**: 그 가정이 신청서에서 고른 "신청한 날"(`Room.hostFrom`·`hostTo`,
  `homeWindowOf`)과 정원을 보여 주고, 며칠을 넣을지 고른다. [전체 기간 배정]은 아이 일정을 그대로
  넣고(나누지 않는다), [이 기간만 배정]은 고른 기간만 넣고 남는 기간을 미배정 조각으로 남긴다.
- **가정이 중간 날짜를 비워 두고 신청하면** 기간만으로는 그 빈 밤이 감춰진다. 그래서 빈틈이 있을
  때만 받을 수 있는 밤 목록을 `Room.hostNights` 에 같이 담는다 (`Attendee.stayNights` 와 같은 규칙).
  판정은 `Room.hostsOn(밤)` 하나로 하고, 자동배정은 `fits()` 에서 이걸 봐서 안 받는 밤에 묵는 아이를
  그 집에 넣지 않는다. 손으로 배정할 때는 막지 않고 **안 받는 날을 짚어 주며 한 번 더 묻는다** —
  운영자가 가정에 전화해서 되는 경우가 있다. 경고는 `_confirmOutside` 하나가 맡고, 가정 한 곳을
  고르는 길도 여러 곳을 골라 한 번에 채우는 길도 모두 이 함수를 지난다.
- **`hostNights` 는 신청서에서만 온다.** 이 칸이 생기기 전에 만들어 둔 가정은 비어 있어서, 비워 둔
  날이 "받는 기간" 안에 삼켜진 채로 남아 있다. `syncHomestayRooms` 가 부를 때마다 다시 맞추므로
  [가정 관리] → [신청에서 가져오기]를 한 번 누르면 채워진다. 신청 기간을 아예 못 가져온 가정은
  배정 창이 집회 전체 기간을 보여주지 않고 그 사실과 누를 버튼을 적는다(`_noWindow`).
...
## 14. 집회별 운영자 (2026-09-19)

집회 하나만 맡는 운영자를 둔다. 큰 집회마다 담당자가 다른데, 지금까지는 전체 운영자 아니면 아무것도 못 했다.

| 구분 | 할 수 있는 것 |
|---|---|
| **전체 운영자** (`admins`) | 모든 집회 · 새 집회 만들기·삭제 · 가입 승인 |
| **집회별 운영자** (`gathering_admins`) | 맡은 집회의 설정·신청·입금·방배정·이미지만 |

- 판정은 `manages(집회id)` = `is_admin() or gathering_admins 에 있음` 하나로 한다. RLS 정책(`gatherings` update,
  `registrations`, `room_plans`, 이미지 버킷)과 운영자 RPC(`save_room_plan`·`admin_add_registration`·`reset_pin`)가
  모두 이 함수를 부른다. 서버에서 막으니 화면을 우회해도 소용없다.
- 이미지는 경로가 `<집회id>/...` 라 첫 칸으로 판정한다.
- 등록은 **집회 설정 → [이 집회의 운영자]** 에서 이메일로. 그 사람은 먼저 운영자 웹에서 [가입 신청]을 해야 한다
  (전체 운영자 승인은 없어도 된다 — 집회 운영자로 등록되면 그 집회는 바로 쓴다). 계정이 없으면 `NO_ACCOUNT`.
- 로그인 문턱은 `can_manage()` (전체 운영자이거나 어느 집회든 맡았으면 통과). 집회 목록은 `my_gatherings()` 로 걸러
  맡은 집회만 보여주고, [새 집회]·[집회 삭제]는 전체 운영자에게만 보인다.
- 같은 집회의 운영자끼리는 동등하다 — 서로를 등록·해제할 수 있다. 계정 승인·거절은 전체 운영자만.

---

## 15. 스탭 신청 · 담당구역 (2026-09-20)

집회를 돕는 **스탭**을 따로 받고, 스탭이 맡을 **담당구역**(해야 할 일)을 만들어 사람을 배정한다.

```
신청 웹: [ ] 스탭으로 신청합니다      ──확정──▶ 참석자(스탭 표시)
                                                      │
운영자: [스탭] → [담당구역] 에서 구역 생성(주방·차량·등록대…)  ──배정──▶ 구역마다 명단·할 일
```
...
| # | 질문 | 이대로 만듦 | 다른 선택지 |
|---|---|---|---|
| 1 | 새 '스탭신청' 칸이 이미 만든 집회에도 보이나 | **안 보인다.** 기본 '안 받음'. 나중에 생긴 기본 항목은 운영자가 켤 때만 나온다 — 신청 받는 중인 집회의 신청서가 말없이 바뀌면 안 된다 | 사역자 칸처럼 기본 '선택'(코드 0줄) |
| 2 | 체크 칸의 '필수'는 무슨 뜻인가 | **반드시 체크해야 신청된다.** 스탭만 받는 집회용. 안 하면 "스탭 신청에 체크해야 합니다" | 필수 설정을 빼고 표시 여부만 |
| 3 | 한 사람이 담당구역 여러 곳을 맡나 | **맡는다.** 주방 + 차량 겸임이 실제로 있다. 구역이 배정된 사람 목록을 들고 있다(`Duty.personIds`) | 방배정처럼 한 곳만(`Attendee.dutyId`) |
| 4 | 배정 후보는 누구인가 | **참석자 전원**, 창 안에 [스탭만] 필터. 스탭 체크를 안 받는 집회에서도 쓴다 | 스탭 체크한 사람만 |
| 5 | 담당구역을 어디에 저장하나 | **방배정 문서(`room_plans`) 안.** `Event.duties` 라 DB 변경이 없고 백업·되살리기에 그냥 따라온다 | 테이블 신설 |
| 6 | 기간을 나눈 참석자(`splitOf`) | **사람 단위로 배정한다** (`personId` 기준). 조각마다 따로 맡을 일이 없다 | 조각별 배정 |

### 15.1 데이터

**`gatherings.shown_fields`** (신규 칸) — 기본이 '안 받음'인 기본 항목 중 운영자가 **켠** 것.
`hidden_fields` 의 반대다. 나중에 생긴 칸은 여기로 들어와 예전 집회를 건드리지 않는다.
...
```dart
// gathering.dart
/// 기본이 '안 받음'인 기본 항목. 목록에 있어야 신청서에 나온다.
static const optInFields = {'staff'};
List<String> shownFields;
bool asks(String f) => optInFields.contains(f)
    ? shownFields.contains(f)
    : !hiddenFields.contains(f);
// requires() 는 그대로 — asks(f) && requiredFields.contains(f)
```

**`Person.staff`** (신규 칸) — 신청서의 "스탭으로 신청합니다" 체크. `people` jsonb 안이라 서버는 안 고친다
(`minister` 와 똑같다). **`Attendee.staff`** 도 같이 두고 `syncRegistrations` 가 옮긴다 —
담당구역 창의 [스탭만] 필터가 이걸 본다.

**`Duty`** (신규 모델, `models.dart`) — 방배정 문서 안에 산다.

```dart
class Duty {
  String id;
  String name;            // 담당구역 이름. 예: 주방, 차량, 등록대
  String? tasks;          // 해야 할 일 (여러 줄)
  int? capacity;          // 필요 인원. null = 정하지 않음
  List<String> personIds; // 배정된 사람 (Attendee.personId)
}
class Event { List<Duty> duties; }   // toJson/fromJson 에 추가, 없으면 빈 목록
```

없는 사람 id 는 화면에서 빼고 보여준다(참석자를 지웠다 되살리는 일이 있다). 참석자를 정말 지울 때는
`Store.deleteAttendee` 가 구역에서도 뺀다.

### 15.2 집회 설정 — 신청서 기본 항목에 한 줄

'출생연도 · 성별 · 셀 · 존' 아래 **스탭신청**을 같은 [안 받음 | 선택 | 필수] 세그먼트로 넣는다.
세그먼트가 `hidden_fields`·`shown_fields`·`required_fields` 중 어디를 고칠지는 `setFieldMode()` 하나가 정한다
(지금 화면에 인라인으로 박혀 있는 코드를 그대로 옮긴 것).

### 15.3 신청 웹

`g.asks('staff')` 면 사역자 체크 바로 위에 체크박스 한 줄. `g.requires('staff')` 면 체크 안 하고 제출하면 막는다.
신청 조회에도 이름 옆에 '스탭'을 적는다. 회비 계산은 건드리지 않는다 — 스탭 할인이 필요하면 지금 있는 지정 할인을 쓴다.

### 15.4 담당구역 화면 (운영자, 새 탭)

왼쪽 메뉴에 세 번째 묶음 **[스탭]** 을 만들고 그 아래 **담당구역** 하나를 둔다.

```
준비   집회 설정 · 신청·입금 · 방 관리 · 참석자
배정   방배정 · 자동배정 · 현황
스탭   담당구역                       ← 추가
```

- 상단 카드: 구역 수 · 배정 인원 · 아직 아무 구역도 없는 스탭 수 + [구역 추가]
- 본문: 구역 카드 목록. 카드 하나에 이름, `배정/필요` 인원, 할 일, 배정된 사람 칩.
  - 카드 제목을 누르면 수정 창(이름·필요 인원·할 일·삭제)
  - [인원] 을 누르면 참석자 고르기 창 — 이름 검색 + [스탭만] 칩 + 여러 명 체크. 이미 다른 구역을 맡은 사람은
    구역 이름을 옆에 적어 준다(겸임은 막지 않는다).
  - 사람 칩의 × 로 바로 뺀다.
- 방배정처럼 고친 뒤 `store.commit()` 한 번. 저장·충돌 처리는 이미 있는 것을 그대로 탄다.

### 15.5 안 만드는 것

- 자동 담당 배정(자동배정은 방만). 구역은 수가 적고 사람을 보고 정한다.
- 스탭 전용 회비·할인 규칙, 담당구역별 일정·시간표, 스탭에게 배정 결과를 보여주는 신청 조회 화면.
- 신청 목록의 [스탭만] 필터 — 지금은 사람 줄에 '스탭'만 적는다. 필요하면 사역자 필터 옆에 칩 하나.

---

## 16. 스탭 페이지 · 담당구역 할 일 (2026-09-20)

담당구역을 맡은 스탭이 **자기 구역의 할 일을 직접 적는다.** 신청 링크처럼 스탭 링크를 따로 주고,
들어온 사람에게는 **자기 담당구역만** 보인다.

```
운영자: 담당구역 만들고 사람 배정 (§15)
        집회 설정에서 [스탭 페이지 링크] 복사 → 스탭들에게 전달
                         │
스탭:   링크 → 휴대폰 + PIN(신청할 때 정한 것) → 내 담당구역 카드
        [+ 행 추가] 구분 · 내용 · 수량 · 체크  → [저장]
                         │
운영자: [스탭] → [담당구역 할일] 에서 모든 구역의 할 일을 한 화면에서 본다(고칠 수도 있다)
```
...
| # | 질문 | 이대로 만듦 | 다른 선택지 |
|---|---|---|---|
| 1 | 스탭은 어떻게 로그인하나 | **신청할 때 쓴 휴대폰 + PIN.** 조회와 같은 관문(`_verify`)을 그대로 탄다 — 5번 틀리면 30분 잠김 | 스탭 전용 비밀번호·초대 코드 |
| 2 | 누가 들어갈 수 있나 | **담당구역이 배정된 사람.** 스탭 체크(`staff`)는 안 본다 — 구역이 없으면 보여 줄 게 없고, 스탭 칸을 안 받는 집회도 쓴다. 붙여넣기로 등록돼 신청이 없는 사람은 못 들어온다(휴대폰+PIN이 없다) | 스탭 체크까지 요구 |
| 3 | 링크는 어떻게 | **신청 웹과 같은 빌드에 `?g=<집회id>&staff=1`.** 배포 주소·CI 를 늘리지 않는다 | 진입점·배포 경로 신설 |
| 4 | 할 일은 어디에 저장하나 | **`Duty.items`** — 방배정 문서 안. 서버는 그 구역의 `items` 한 칸만 바꾼다(`save_duty_items`) | 테이블 신설 |
| 5 | 수량은 숫자인가 | **문자열.** "3", "2박스", "넉넉히" 가 다 들어간다. 합계를 내지 않는다 | 정수 + 단위 칸 |
| 6 | 스탭 둘이 같은 구역을 동시에 고치면 | **나중에 저장한 쪽이 이긴다.** 구역 하나의 `items` 만 덮어쓰므로 다른 구역·방배정은 안전하다. 구역 하나를 여럿이 동시에 쓰는 일은 드물다 | 버전 검사(`CONFLICT`) |
| 7 | 운영자도 고칠 수 있나 | **고칠 수 있다.** 스탭 페이지와 운영자 화면이 같은 표 위젯(`widgets/duty_tasks.dart`)을 쓴다 | 운영자는 보기만 |

### 16.1 데이터

**`DutyTask`** (신규 모델, `models.dart`) — 표 한 줄. `Duty.items` 에 담긴다.

```dart
class DutyTask {
  String id;
  String kind;     // 구분 (예: 준비물 / 예산 / 확인)
  String content;  // 내용
  String qty;      // 수량 — 문자열
  bool done;       // 체크
}
class Duty { ...; List<DutyTask> items; }
```

`Duty.tasks`(운영자가 적는 구역 설명)와 `Duty.items`(스탭이 채우는 표)는 다른 것이다.
`models.dart` 와 `widgets/duty_tasks.dart` 가 신청 웹 빌드에도 들어가므로 `dart:io` 를 쓰지 않는다.

### 16.2 서버 (`schema.sql`)

신청자는 `room_plans` 에 못 닿으므로 RPC 두 개를 새로 연다 (신청 RPC 와 같은 자리).

```sql
-- 내 담당구역과 할 일. 휴대폰+PIN 이 틀리면 null, 맡은 구역이 없으면 빈 배열.
lookup_duties(p_gathering uuid, p_phone text, p_pin text) returns jsonb
--   → {"name": "<신청자 이름>", "duties": [{id, name, tasks, capacity, items, members: ["이름", …]}]}

-- 그 구역의 할 일 표만 바꾼다. 내가 배정된 구역이 아니면 FORBIDDEN.
save_duty_items(p_gathering uuid, p_phone text, p_pin text, p_duty text, p_items jsonb) returns jsonb
```

- 판정: 신청의 `people[].id` 가 곧 참석자의 `personId` 라 **신청 한 건만 보면 된다**
  (`duties[].personIds` 와 겹치는지). 취소된 신청은 들어갈 수 없다.
- `items` 는 200줄·20,000자까지 (`INVALID_ITEMS`). 저장하면 문서 `version` 이 올라가서
  운영자 앱이 다음 저장 때 `CONFLICT` 로 알아채고 다시 읽는다.
- 같이 일하는 사람 이름(`members`)은 주고 연락처·메모는 주지 않는다.

### 16.3 스탭 페이지 (신청 웹, `main_public.dart`)

`?g=<집회id>&staff=1` 로 들어오면 집회 페이지 대신 스탭 페이지가 뜬다.
휴대폰 + PIN → 내 구역 카드마다 [구분 | 내용 | 수량 | ✓] 표와 [+ 행 추가] · [저장].
맡은 구역이 없으면 "배정된 담당구역이 없습니다. 담당자에게 문의하세요."

### 16.4 운영자 화면 — [스탭] → 담당구역 할일

- 위 카드: 구역 수 · 할 일 수 · 완료 수 + [스탭 페이지 링크 복사]
- 구역마다 섹션: 이름 · 배정 인원 · `완료/전체` + 같은 표 위젯(고칠 수 있다)
- 집회 설정의 신청 링크 아래에도 스탭 페이지 링크를 한 줄 적는다.

### 16.5 안 만드는 것

- 스탭별 알림·마감일·담당자 지정(표 한 줄을 누가 맡는지), 사진 첨부, 구역 간 이동.
- 할 일 변경 이력. 누가 언제 고쳤는지는 남기지 않는다.
- 스탭 페이지에서의 구역 만들기·인원 배정 — 그건 운영자 화면에서만 한다.
